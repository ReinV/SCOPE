...
def return_code(widget):
    ...
    if widget == 'slider1':
        ...
        code = """
            ...
            if (checkbox.value == 0){
                var new_data = term_to_source[term]['source_norm'][value].data;
            } else {
                var new_data = term_to_source[term]['source'][value].data;
            }
        ...
        code = """
            ...
            // apply current level of saturation
            for (var i = 0; i < source_data['counts'].length; i++) {
            source_data['scaling'][i] = Math.pow(source_data['blur'][i], 1/power)
            }
...
def make_plot_sources(tables, size, ratio, orientation, blur_max, step_size):
    ...
    for term in tables.keys():
        ...
        term_to_source[term] = {'source': source, 'source_norm': source_norm, 'title': title}
...
def plot(tables, output_filename):
    ...
    'source': source, 'mapper': mapper, 'slider1': slider1, 'multi_select': multi_select,
    'checkbox': checkbox, 'term_to_source': term_to_source, 'step_size': BLUR_STEP_SIZE}, code=code_callback_slider2)
    callback_checkbox = CustomJS(args={
    'source': source, 'term_to_source': term_to_source, 'multi_select': multi_select, 'step_size': BLUR_STEP_SIZE,
    ...
    'source': source, 'term_to_source': term_to_source, 'checkbox': checkbox, 'metadata': metadata, 'step_size': BLUR_STEP_SIZE,
    'slider2': slider2, 'slider1': slider1, 'p': p, 'mapper': mapper}, code=code_callback_ms)
