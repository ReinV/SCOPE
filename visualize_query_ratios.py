...
def hex_to_bin(x, y, size, aspect_scale):
    # pd = import_required('pandas','hexbin requires pandas to be installed')

    q, r = cartesian_to_axial(x, y, size, orientation="pointytop", aspect_scale=aspect_scale)
    print(len(q))
    print(len(r))
...
def create_array(table):
    # create lists
    x = [[float(logP)]*count for logP, count in zip(table.logP, table.Count)]
    y = [[float(mass)]*count for mass, count in zip(table.Mass, table.Count)]
    names = [[name]*count for name, count in zip(table.Names, table.Count)]
    # flatten lists
...
def get_ratio(x, y, t_x, t_y, lb, normalize):
...
def calculate_ratios(df, dfs, lb, normalize):
    ...
    total_x = df_merged.counts_x.sum()
    total_y = df_merged.counts_y.sum()
    print(total_x, total_y)
    df_merged.loc[:,'ratio'] = [get_ratio(x, y, total_x, total_y, lb, normalize) for x, y in zip(df_merged.counts_x, df_merged.counts_y)]
...
def plot_ratio(x, y, names, xs, ys, names_s, lb, query_1, query_2, normalize):
    ...
    # calculate ratio's
    df_ratio_low, df_ratio_high, minimum, maximum = calculate_ratios(df1, df2, lb, normalize)
...
def main():
    ...
    plot_ratio(x, y, names, xs, ys, names_s, LOWER_BOUND, query_1, query_2, normalize)
